"""
Gerador de Gráficos e Visualizações Comparativas.

Este módulo processa os arquivos de estatísticas CSV gerados pelas simulações
e produz gráficos individuais e comparativos em formato PNG nas pastas:
  - graphs/individual/<politica>/ (Vazão vs Qualidade e Nível do Buffer)
  - graphs/comparison/ (IQS, Bitrate Acumulado, Grid de Correlação e Jitter EWMA)
"""
...
def generate_individual_graphs(csv_path, output_base_dir, policy_name):
    """
    Gera gráficos individuais detalhados para uma política específica, incluindo marcadores de Failover.

    Args:
        csv_path (str): Caminho do arquivo CSV de métricas da política.
        output_base_dir (str): Diretório base de saída dos gráficos.
        policy_name (str): Nome da política (ex: 'baseline', 'buffer', 'heuristic').
    """
    ...
    # Identifica os segmentos onde ocorreu Failover (mudança do ID do servidor)
    failover_segments = []
    ...
    # --- 1. Vazão vs Qualidade (com eixos múltiplos e linhas de Failover) ---
    ...
    # Segundo eixo Y para as resoluções de vídeo
    ax2 = ax1.twinx()
...
def generate_iqs_comparison(df_base, df_buff, df_heur, output_dir):
    """
    Gera o gráfico comparativo do Índice de Qualidade Segura (IQS) com saturação em 20s.

    Args:
        df_base (pd.DataFrame): Dados da política Baseline.
        df_buff (pd.DataFrame): Dados da política Buffer-Based.
        df_heur (pd.DataFrame | None): Dados da política Heurística.
    ...
    SATURATION_LIMIT = 20.0  # Limite máximo de saturação do buffer em segundos
    ...
    plt.fill_between(df_buff['segment'], df_buff['iqs'], color='green', alpha=0.3, label='Buffer-Based (Eficiência)')
...
def generate_cumulative_bitrate_comparison(df_base, df_buff, df_heur, output_dir):
    """
    Gera gráfico comparativo de Bitrate Acumulado (reinicia a contagem se buffer_can_play == 0).
    ...
    plt.plot(df_buff['segment'], get_cumulative(df_buff), label='Buffer-Based (Acumulado)', color='green', linewidth=2)
    if df_heur is not None:
...
def generate_correlation_grid(df_base, df_buff, df_heur, output_dir):
    """Gera grade comparativa de correlação entre Nível de Buffer e Bitrate Selecionado."""
    ...
    dfs = [df_base, df_buff]
    titles = ['BASELINE (Rate-Based)', 'POLÍTICA BUFFER-BASED']
    if has_heur:
        dfs.append(df_heur)
        titles.append('POLÍTICA HEURÍSTICA')
...
def generate_jitter_comparison(df_base, df_buff, df_heur, output_dir):
    """Gera gráfico comparativo da variação de Jitter EWMA ao longo dos segmentos."""
    plt.figure(figsize=(12, 6))
    ...
    plt.plot(df_buff['segment'], df_buff['jitter_ewma_ms'], label='Buffer-Based', color='green', linewidth=2, alpha=0.7)
...
def generate_comparison_graphs(csv_baseline, csv_buffer, csv_heuristic, output_base_dir):
    """
    Função orquestradora que gera todos os gráficos comparativos.

    Args:
        csv_baseline (str): Caminho para o CSV da política Baseline.
        csv_buffer (str): Caminho para o CSV da política Buffer-Based.
        csv_heuristic (str): Caminho para o CSV da política Heurística.
        output_base_dir (str): Diretório base para salvar as imagens dos gráficos.
    """
